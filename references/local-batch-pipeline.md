...
Sci-Hub 使用独立 60 秒预算，不消耗前序渠道的剩余时间；可用 `SCANSCI_SCIHUB_SECONDS`
单独调整为正数秒，仍保留请求超时和失败退出。
...
- 每篇科研通求助在一次运行中默认最多等应助 300 秒（`--ablesci-wait-seconds`）；
  内部每轮最多 45 秒、约每 30 秒检查，`--until-complete` 不会重置该篇时限。
  超时记录 `wait_limit_reached`，保留原求助和账本，不撤单、不重发、不自动加价；
  继续其他未超时任务，剩余仅为超时项时以未完成退出。页面读取本身可能带来少量延迟。
  后续明确续跑时查询同一求助，重新提供一个有限等待窗口，不新增悬赏。
  该限制只管尚无人上传的等待，不中断已开始的下载、核验和导入。
  观察工具超时不等于进程退出，不可重复启动。跨会话定时监控
